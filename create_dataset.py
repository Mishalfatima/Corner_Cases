...
parser.add_argument('--output_path', type=str, default='/ceph/mfatima/corner_cases/Hard-Spurious-ImageNet/test/', help='Path to the output directory')
...
def resize_to_square(img, target_size):
    """Resize image to a square of target_size x target_size, ignoring aspect ratio."""
    pil_img = Image.fromarray(img).convert('RGB')
    resized = np.array(pil_img.resize((target_size, target_size))) / 255.
    return resized

def utils(img_size, bg, obj_bbox, obj_bbox_seg=None):
    H, W = img_size, img_size

    # obj_bbox is already square: (target_size, target_size, 3)
    ...
    new_image[start_x:end_x, start_y:end_y, :] = obj_bbox[0:end_x - start_x, 0:end_y - start_y, :]
...
for c, j in enumerate(classes):
    ...
    for cc, i in enumerate(images):
        try:
            ...
            # Resize object bbox to exact squares — no AR preservation
            obj_bbox1 = resize_to_square(obj_bbox, obj_size[0])  # 56x56
            obj_bbox2 = resize_to_square(obj_bbox, obj_size[1])  # 84x84
            obj_bbox3 = resize_to_square(obj_bbox, obj_size[2])  # 112x112
            ...
            for bbox, size in zip([obj_bbox1, obj_bbox2, obj_bbox3], obj_size):
                center, corner = utils(img_size, bg, bbox, None)
